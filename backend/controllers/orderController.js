  ...
  try {
    ...
    (async () => {
      try {
        ...
        if (!to) {
          console.warn(
            `Order ${orderId}: no email found to send status update`,
          );
          return;
        }
...
const COMPANY = {
  gstin: "UDYAM-GJ-03-0051710",
  email: "layerly2024@gmail.com",
  phone: "+91 96648 51323",
  logoUrl:
...
const downloadInvoice = async (req, res) => {
  try {
    ...
    doc
      ...
      .moveDown(0.2)
      .fontSize(10)
      .text(`OrderID: #${order._id.toString().slice(-8)}`, metaX, doc.y, {
        width: metaW,
        align: "right",
      })
      .text(
      ...
      .text(`PhonePe Txn: ${order.phonePeTxnId || "-"}`, metaX, doc.y, {
        width: metaW,
        align: "right",
      });
    ...
    doc
      .moveTo(contentLeft, headerBottom + 10)
    ...
    doc.fontSize(12).fillColor("#000").text("Bill To:", contentLeft, y);
    y = doc.y + 4;
    ...
    const billToName =
      ...
      "";
    if (billToName)
      doc.text(billToName, contentLeft, y, { width: contentWidth / 2 });
    if (order.address?.email) doc.text(order.address.email);
    if (order.address?.street) doc.text(order.address.street);
    doc.text(
      `${order.address?.city || ""}${order.address?.city ? ", " : ""}${order.address?.state || ""} ${order.address?.pinCode || ""}`.trim(),
    );
    ...
    const colQtyX = contentLeft + contentWidth * 0.6;
    ...
    const drawTableHeader = () => {
      ...
      doc
        .text("Item", colItemX, headerY, { width: colQtyX - colItemX - 10 })
        .text("Qty", colQtyX, headerY, {
          width: colPriceX - colQtyX - 10,
          align: "right",
        })
        .text("Price", colPriceX, headerY, {
    ...
    for (const it of order.items) {
      ...
      doc
        .text(it.name || it.productId || "-", colItemX, rowY, {
          width: colQtyX - colItemX - 10,
        })
        .text(String(qty), colQtyX, rowY, {
          width: colPriceX - colQtyX - 10,
          align: "right",
        })
        .text(formatINR(price), colPriceX, rowY, {
          width: colTotalX - colPriceX - 10,
          align: "right",
        })
        .text(formatINR(line), colTotalX, rowY, {
          width: colTotalW,
          align: "right",
        });
